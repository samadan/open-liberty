...
import org.jboss.logging.Logger;  //Liberty change
...
public class GeneralValidatorImpl implements GeneralValidatorCDI {
   ...
   private static final Logger log = Logger.getLogger(GeneralValidatorImpl.class.getName()); //Liberty change
    ...
    public GeneralValidatorImpl(final ValidatorFactory validatorFactory, final boolean isExecutableValidationEnabled,
    ...
    public void checkViolationsfromCDI(HttpRequest request) {
        ...
        SimpleViolationsContainer violationsContainer = SimpleViolationsContainer.class
                .cast(request.getAttribute(SimpleViolationsContainer.class.getName()));
        if (violationsContainer != null && violationsContainer.size() > 0) {
            ResteasyViolationExceptionImpl ViolationException = new ResteasyViolationExceptionImpl(violationsContainer, request.getHttpHeaders().getAcceptableMediaTypes());  //Liberty change
            log.warn(ViolationException.getLocalizedMessage());  //Liberty change
            throw ViolationException;  //Liberty change
    ...
    public void validateAllParameters(HttpRequest request, Object object, Method method, Object[] parameterValues,
        ...
        if ((violationsContainer.isFieldsValidated()
                ...
                || GetRestful.isSubResourceClass(object.getClass()) //Liberty change
    ...
    ResteasyConstraintViolation createResteasyConstraintViolation(ConstraintViolation<?> cv, Type ct) {
        ...
        ResteasyConstraintViolation rcv = new ResteasyConstraintViolation(ct, path, cv.getMessage(),
    ...
    private static boolean hasEJBScope(Class<?> clazz) {
        return classHasAnnotations(clazz,
                new String[] { "jakarta.ejb.Stateless", "jakarta.ejb.Stateful", "jakarta.ejb.Singleton" });
    }
