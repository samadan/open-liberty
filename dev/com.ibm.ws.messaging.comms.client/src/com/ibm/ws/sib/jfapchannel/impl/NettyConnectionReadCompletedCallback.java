...
public class NettyConnectionReadCompletedCallback extends BaseConnectionReadCallback
{
	...
	static
	{
		if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) SibTr.debug(tc, "@(#) SIB/ws/code/sib.jfapchannel.client.common.impl/src/com/ibm/ws/sib/jfapchannel/impl/NettyConnectionReadCompletedCallback.java, SIB.comms, WASX.SIB, uu1215.01 1.58");
	}
	 ...
	 * @param conv
	 */	
	protected NettyConnectionReadCompletedCallback(Connection c,
			boolean onClient,
			AcceptListener al,
	...
	throws SIResourceException
	{
		...
		tcpCtx = (NettyIOConnectionContext)x;
		readCtx = (NettyIOReadRequestContext)x.getReadInterface();
	...
	public void readCompleted(WsByteBuffer buff, IOReadRequestContext rctx, NettyNetworkConnection vc) {
		...
		catch(Error error)
		{
			...
			(error, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionReadCompletedCallback", JFapChannelConstants.CONNREADCOMPCALLBACK_COMPLETE_01, thisConnection.getDiagnostics(true));
			...
			thisConnection.invalidate(false, error, "Error caught in NettyConnectionReadCompletedCallback.complete()");
			...
			IOException t)               // F176003, F184828, D194678
	{
			...
			synchronized(this)
			{
				...
				else if (t instanceof SocketTimeoutException)
				{
					...
					if (awaitingHeartbeatResponse.isSet())
					{
						...
						FFDCFilter.processException
						(exception, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionReadCompletedCallback", JFapChannelConstants.CONNREADCOMPCALLBACK_ERROR_03, thisConnection.getDiagnostics(true));
		...
		{
			FFDCFilter.processException
			(error, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionReadCompletedCallback", JFapChannelConstants.CONNREADCOMPCALLBACK_ERROR_05, thisConnection.getDiagnostics(true));
			if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) SibTr.exception(this, tc, error);
			...
			thisConnection.invalidate(false, error, "Error caught in NettyConnectionReadCompletedCallback.error()");
		...
		{
			FFDCFilter.processException
			(runtimeException, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionReadCompletedCallback", JFapChannelConstants.CONNREADCOMPCALLBACK_ERROR_06, thisConnection.getDiagnostics(true));
			if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) SibTr.exception(this, tc, runtimeException);

			// We can reasonably try to recover from a runtime exception by invalidating the associated
			// connection.  This should drive the underlying TCP/IP socket to be closed.
			thisConnection.invalidate(false, runtimeException, "RuntimeException caught in NettyConnectionReadCompletedCallback.error()");
