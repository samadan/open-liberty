...
public class NettyConnectionWriteCompletedCallback extends BaseConnectionWriteCallback
{
	 ...
	 */
	public NettyConnectionWriteCompletedCallback(PriorityQueue priorityQueue, IOWriteRequestContext x, // F184828
			Connection connection) throws FrameworkException // F176003
	{
		...
		writeCtx = (NettyIOWriteRequestContext)x;
	...
	public void complete(NetworkConnection vc, IOWriteRequestContext wctx) {
		...
		try {
			...
			(error, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionWriteCompletedCallback", JFapChannelConstants.CONNWRITECOMPCALLBACK_COMPLETE_03, connection.getDiagnostics(true));
			...
			connection.invalidate(false, error, "Error caught in NettyConnectionWriteCompletedCallback.complete()");
			...
			FFDCFilter.processException
			(runtimeException, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionWriteCompletedCallback", JFapChannelConstants.CONNWRITECOMPCALLBACK_COMPLETE_04, connection.getDiagnostics(true));
			...
			connection.invalidate(false, runtimeException, "RuntimeException caught in NettyConnectionWriteCompletedCallback.complete()");
				...
				if (dequeueTransmissionData(writeBuffer)) {
					synchronized (connectionClosedLock) {
						if (!connectionClosed) {
							...
							final NetworkConnection vc = writeCtx.write(writeBuffer, this);
	...
	private void notifyReadySendListeners(BlockingQueue<Pair<SendListener, Conversation>> readySendCallbacks) {
		...
		try {
			...
			FFDCFilter.processException
			(t, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionWriteCompletedCallback", JFapChannelConstants.CONNWRITECOMPCALLBACK_COMPLETE_01, connection.getDiagnostics(true));
			if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled())
	...
	public void error(NetworkConnection vc, IOWriteRequestContext wrc, IOException t) {
		...
		try {
			...
			FFDCFilter.processException
			(error, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionWriteCompletedCallback", JFapChannelConstants.CONNWRITECOMPCALLBACK_COMPLETE_01, connection.getDiagnostics(true));
			if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) SibTr.exception(this, tc, error);
			...
			connection.invalidate(false, error, "Error caught in NettyConnectionWriteCompletedCallback.error()");
			...
			FFDCFilter.processException
			(runtimeException, "com.ibm.ws.sib.jfapchannel.impl.NettyConnectionWriteCompletedCallback", JFapChannelConstants.CONNWRITECOMPCALLBACK_COMPLETE_05, connection.getDiagnostics(true));
			if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) SibTr.exception(this, tc, runtimeException);

			// We can reasonably try to recover from a runtime exception by invalidating the associated
			// connection.  This should drive the underlying TCP/IP socket to be closed.
			connection.invalidate(false, runtimeException, "RuntimeException caught in NettyConnectionWriteCompletedCallback.error()");
